...
vagrant ssh client <<EOF
    ...
    sudo ip mptcp endpoint add 192.168.57.100 dev eth2 subflow fullmesh
    sudo ip mptcp endpoint add 192.168.58.100 dev eth3 subflow fullmesh
EOF
...
vagrant ssh server <<EOF
    ...
    # Enable MPTCP on nginx
    # sudo mptcpize enable nginx
EOF
